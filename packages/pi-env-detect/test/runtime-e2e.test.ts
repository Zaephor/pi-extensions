/**
 * e2e test for pi-env-detect — install via pi-monorepo-registry, then assert
 * the extension symlink and package.json shape land correctly under the pi
 * runtime's agent dir.
 *
 * pi-env-detect is installed via the registry (not natively).
 */
...
describe("Install pi-env-detect via pi", () => {
	...
	it("extension directory contains package.json with pi.extensions", () => {
		const extDir = getExtensionsDirFor(piAgentDir);
		const pkgJsonPath = path.join(extDir, "pi-env-detect", "package.json");

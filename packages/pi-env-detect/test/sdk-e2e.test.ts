/**
 * SDK e2e test — uses pi's real SDK to load the pi-env-detect extension
...
beforeAll(async () => {
	tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-env-detect-e2e-"));
...
describe("SDK e2e — pi-env-detect extension via createAgentSession", () => {
	...
	it("extension resolves to correct path", () => {
		const matching = extensionsResult.extensions.filter(
			(ext) => ext.path.includes("pi-env-detect") || ext.resolvedPath.includes("pi-env-detect"),
		);

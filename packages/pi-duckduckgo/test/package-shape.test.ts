...
async function npmPack(): Promise<PackEntry> {
	tmpDir = mkdtempSync(join(tmpdir(), "pi-duckduckgo-pack-"));
...
/** Read and parse the package.json from the pi-duckduckgo package. */
...
describe("package shape", () => {
	...
	describe("tarball contents", () => {
		...
		it("includes src/search.ts", () => {
			expect(filePaths).toContain("src/search.ts");
		});

		it("includes src/format.ts", () => {
			expect(filePaths).toContain("src/format.ts");
		});
